...
class ABTest(TimestampMixin, Base):
    ...
    baseline_snapshot_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)
    variant_snapshot_id: Mapped[uuid.UUID | None] = mapped_column(Uuid, nullable=True)
...
class ABTestMetricSnapshot(Base):
    ...
    ab_test_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("ab_tests.id"), nullable=False)
...
class ABTestResult(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    ab_test_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("ab_tests.id"), unique=True, nullable=False)
